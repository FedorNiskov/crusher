...
{
  if (on_off) {
    gsm_printf("[GSM] power on\n");
  } else {
    gsm_printf("[GSM] power off\n");
  }
}
...
int sscanf(const char *str, const char *fmt, char *buf)
{
  int p = 0;
  while (fmt[p] != '%') {
    ++p;
...
bool atc_command(atc_t *atc, const char *cmd, int num, char *buf, ...)
{
  strcpy(buf, "\r\n+CPIN: READY");
  return true;
}
